(function(module) {
  ...
  Article.createTable = function(callback) {
    webDB.execute ('CREATE TABLE IF NOT EXISTS article_table (id INTEGER PRIMARY KEY, title VARCHAR(255) NOT NULL, author VARCHAR(255) NOT NULL, authorUrl VARCHAR (255), category VARCHAR(20), publishedOn DATETIME, body TEXT NOT NULL);',
      function(result) {
  ...
  Article.truncateTable = function(callback) {
    webDB.execute('DELETE FROM article_table;',callback);
  };

  // DONE: Insert an article instance into the database:**
  Article.prototype.insertRecord = function(callback) {
    webDB.execute([{
      'sql': 'INSERT INTO article_table (title, author, authorUrl, category, publishedOn, body) VALUES (?, ?, ?, ?, ?, ?);',
  ...
  Article.prototype.deleteRecord = function(callback) {
    webDB.execute([{
      'sql':'DELETE FROM article_table WHERE id = ?;',
      'data': '[this.id]'
    }],callback);
  };
  ...
  Article.prototype.updateRecord = function(callback) {
    webDB.execute([{
      'sql': 'UPDATE article_table SET title = ?, author = ?, authorUrl = ?, category = ?, publishedOn = ?, body = ? WHERE id = ?;',
  ...
  Article.fetchAll = function(next) {
    webDB.execute('SELECT * FROM article_table ORDER BY publishedOn DESC', function(rows) {
      if (rows.length) {
        // Now instanitate those rows with the .loadAll function,
        Article.loadAll(rows);
        // and pass control to the view by calling whichever function argument was passed in to fetchAll..
        next();
      } else {
        $.getJSON('data/hackerIpsum.json', function(rawData) {
          ...
          rawData.forEach(function(item) {
            ...
            Article.prototype.insertRecord(callback);
          });
          // Now get ALL the records out the DB, with their database IDs:
          webDB.execute('SELECT * FROM article_table WHERE id = ?;', function(rows) {
